...
from plugin_root import vaws_local_dir, SKILLS_DIR
...
STATE_DIR = vaws_local_dir() / "remote-toolbox"
...
def _load_inventory(repo_root: Path = None) -> tuple[dict[str, Any], Path]:
    if repo_root is None:
        repo_root = vaws_local_dir().parent
...
def _find_machine_record(identifier: str, repo_root: Path = None) -> tuple[dict[str, Any], Path]:
    if repo_root is None:
        repo_root = vaws_local_dir().parent
...
def _run_json_command(cmd: list[str], *, cwd: Path = None, relay_stderr: bool = True) -> tuple[int, dict[str, Any], str, str]:
    if cwd is None:
        cwd = vaws_local_dir().parent
...
def parity_derived_args(target: RemoteTarget, *, force_reinstall: bool = False) -> dict[str, Any]:
    script = SKILLS_DIR / "remote-code-parity" / "scripts" / "parity_sync.py"
...
def _parity_plan_manifest(derived: dict[str, Any]) -> dict[str, Any]:
    script = SKILLS_DIR / "remote-code-parity" / "scripts" / "remote_code_parity.py"
    cmd = [
        sys.executable,
        str(script),
        "plan",
...
def _consent_state(derived: dict[str, Any]) -> dict[str, Any]:
    scripts = SKILLS_DIR / "remote-code-parity" / "scripts"
...
def call_service(action: str, target: RemoteTarget, extra_args: list[str]) -> dict[str, Any]:
    started_at = now_iso()
    start = time.monotonic()
    scripts = SKILLS_DIR / "vllm-ascend-serving" / "scripts"
    script_map = {
    ...
    *,
    ...
    if session_container:
        if not target.session_id:
            ...
            script = SKILLS_DIR / "session-management" / "scripts" / "session_remove.py"
