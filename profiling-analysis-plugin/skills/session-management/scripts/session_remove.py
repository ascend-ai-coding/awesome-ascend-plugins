...
_LIB_DIR = Path(__file__).resolve().parents[3] / "agents_shared" / "lib"
_MM_SCRIPTS = Path(__file__).resolve().parents[3] / "agents_shared" / "machine-management"
_SKILLS_DIR = Path(__file__).resolve().parents[2]
for _p in (str(_LIB_DIR), str(_MM_SCRIPTS)):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from plugin_root import PLUGIN_ROOT, SKILLS_DIR, vaws_local_dir
...
def run_git(args: list[str], *, check: bool = False) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["git", "-C", str(vaws_local_dir().parent), *args],
        capture_output=True,
        text=True,
        check=False,
    )
...
def stop_session(session_id: str, *, session_file: Path | None = None, force: bool) -> dict[str, Any]:
    script = SKILLS_DIR / "vllm-ascend-serving" / "scripts" / "serve_stop.py"
    ...
    if force:
        cmd.append("--force")
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=str(vaws_local_dir().parent), check=False)
