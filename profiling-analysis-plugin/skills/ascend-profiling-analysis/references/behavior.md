...
1. **Remote mode** (`--manifest` or `--remote-profile-root`): The traditional
   mode where profiling data lives on a remote NPU machine. Requires SSH
   connectivity, parity sync, and artifact pull.

2. **Local mode** (`--local-profile-root`): When the user provides a local
   profiling root directory, analysis runs directly on the local machine.
   No SSH, no parity sync, no artifact pull. All output artifacts are written
   directly to the local output directory. This mode is useful when the user
   has already downloaded profiling data to their workstation.
...
1. **Resolve input**: `--local-profile-root <abs-path>` → local directory containing Ascend profiler output (e.g. `kernel_details.csv`).
2. **Local analyze**: run `ascend_profile.analyze` directly as a local subprocess. No SSH, no parity sync.
3. **Validate artifacts**: same required artifact set as remote mode, checked in the local output directory.
4. **Emit JSON** on stdout. Progress lines go to stderr.
