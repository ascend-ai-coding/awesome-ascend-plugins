...
- **远端解析（远程模式）**：profiling root 通常几十 GB，禁止全量拉回本地解析。本地只做静态检查、schema 校验、产物 manifest 阅读。真实 analyze 在远端容器里跑，必要时把 `report/` 目录拉回本地。
- **本地解析（本地模式）**：当用户提供 `--local-profile-root` 时，分析直接在本机运行，不需要远程服务器。适用于用户已经把 profiling 数据下载到本地的场景。
...
- **No remote machine needed**: `--machine`, `--session-id`, `--session-file` are not required and are ignored when `--local-profile-root` is used.
- The profiling root must be a local directory containing the Ascend profiler output (e.g. `kernel_details.csv`, `trace_view.json`, etc.).
- Analysis runs directly as a local subprocess — no SSH, no parity sync, no artifact pull.
- All output artifacts are written directly to the local output directory.
- The same stage selection flags (`--from-stage`, `--to-stage`, `--only-stage`) and report flags (`--skip-html`, `--report-mode`) are supported.
...
### Remote mode (--manifest or --remote-profile-root)
...
### Local mode (--local-profile-root)

1. **确认输入来源**
   - `--local-profile-root`：本地目录路径，必须包含 Ascend profiler 产物（如 `kernel_details.csv`）。
   - 不需要远程机器，跳过 SSH、parity sync、artifact pull。
2. **执行分析**
   - 直接在本机运行 `ascend_profile.analyze` 作为本地子进程。
   - 产物直接写入本地输出目录。
3. **校验产物**
   - 同远程模式的必备文件清单。
4. **回答用户**
...
### profile_analyze.py 单 root (remote mode)
...
### profile_analyze.py 单 root (local mode)

```json
{
  "status": "ok",
  "mode": "local",
  "local_profile_root": "/home/user/profiling_data/20260507_xxx",
  "local_output_dir": ".vaws-local/profiling-analysis/runs/20260507_xxx",
  "stage_timings": [{"stage": "normalize", "elapsed_s": 12.3}, ...],
  "rank_count": 4,
  "event_count": 1234567,
