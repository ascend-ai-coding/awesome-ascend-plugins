...
COLLECTION_STATE_DIR = vaws_local_dir() / "ascend-profiling-collection" / "runs"
SAFE_TOKEN_RE = re.compile(r"[^A-Za-z0-9_.-]+")
SERVING_SCRIPTS = SKILLS_DIR / "vllm-ascend-serving" / "scripts"
...
def call_json_command(cmd: list[str], *, cwd: Path | None = None) -> dict[str, Any]:
    ...
    proc = subprocess.Popen(
        ...
        stderr=subprocess.PIPE,
        text=True,
        cwd=str(cwd or vaws_local_dir().parent),
    )
